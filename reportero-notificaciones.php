<?php include ('./templates/header.php')?>
<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="css/bootstrap.css">
    
    <!-- Style -->
    <link rel="stylesheet" href="css/styles/style.css">
    <link rel="stylesheet" href="assets/css/style.css">

    <title>Crear nota</title>
  </head>

<body>
  

  <div class="container-box">
  <div class="text-center">
    
              <h3 class="text-center mt-5">Reportero</h3>
            </div>

<div class="container-search-result">
  <div class="card-width">

    <div class="card border-secondary mb-3" >
                    <div class="card-header">Categoria</div>
                      <div class="card-body">
                      <div class="row">
                      <div class="col-sm-4">
                        <div class="position-relative image-hover">
                          <img src="./assets/images/news/news-6.jpg" class="img-fluid img-vista"/>
                        </div>
                      </div>
                      <div class="col-sm-8">
                        <div class="position-relative image-hover">
                          <h5 >Ejemplo de titulo de noticia 1</h5>
                          <p class="fs-15"> Autor | Fecha de publicación</p>
                          <p class="text-danger">Comentario de administrador</p>
                        </div>
                        <button  class="btn btn-outline-warning btn-sm ml-1 shadow-none align-right" onclick="location.href='./publish-article.php';">Editar</button>
                      </div>
                    </div>
                      </div>
                  </div>
                </div>



          <div class="card border-secondary mb-3" >
              <div class="card-header">
                  
              Categoria
              
              </div>
                  <div class="card-body">
                      <div class="row">
                          <div class="col-sm-4">
                            <div class="position-relative image-hover">
                              <img src="./assets/images/news/news-7.jpg" class="img-fluid img-vista"/>
                            </div>
                          </div>
                          <div class="col-sm-8">
                            <div class="position-relative image-hover">
                              <h5 >Ejemplo de titulo de noticia 2</h5>
                              <p class="fs-15"> Autor | Fecha de publicación</p>

                          <p class="text-danger">Comentario de administrador</p>
                        </div>
                        <button  class="btn btn-outline-warning btn-sm ml-1 shadow-none align-right" onclick="location.href='./publish-article.php';">Editar</button>
                      </div>
                  </div>
              </div>
            </div>


            
    <div class="card border-secondary mb-3" >
                    <div class="card-header">Categoria</div>
                      <div class="card-body">
                      <div class="row">
                      <div class="col-sm-4">
                        <div class="position-relative image-hover">
                          <img src="./assets/images/news/news-6.jpg" class="img-fluid img-vista"/>
                        </div>
                      </div>
                      <div class="col-sm-8">
                        <div class="position-relative image-hover">
                          <h5 >Ejemplo de titulo de noticia 1</h5>
                          <p class="fs-15"> Autor | Fecha de publicación</p>
                          <p class="text-danger">Comentario de administrador</p>
                        </div>
                        <button  class="btn btn-outline-warning btn-sm ml-1 shadow-none align-right" onclick="location.href='./publish-article.php';">Editar</button>
                      </div>
                    </div>
                      </div>
                  </div>
                </div>
  </div>
</div>        
</div>
</div>
</body>

<?php include ('./templates/footer.php')?>